...
class RequestGroupService {
  constructor() {
    this.configPath = path.join(__dirname, '../../config/request-groups.json');
  ...
  loadConfig() {
    try {
      ...
      console.error('加载请求分组配置失败:', err.message);
    }
  ...
  _processBody(body, config) {
    ...
    try {
      ...
      console.error('处理请求体失败:', err.message);
    }
...
module.exports = new RequestGroupService();
